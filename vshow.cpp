...
void *vs_open (int v_width, int v_height)
{
	...
	AVPixelFormat target_pix_fmt = AV_PIX_FMT_NONE;
	switch (ctx->image->bits_per_pixel) {
		case 32:
			target_pix_fmt = AV_PIX_FMT_RGB32;
			break;
		case 24:
	...
	ctx->curr_width = cx;
	ctx->curr_height = cy;
	ctx->sws = sws_getContext(v_width, v_height, AV_PIX_FMT_YUV420P,
...
int vs_show (void *ctx, unsigned char *data[4], int stride[4])
{
	...
	if (cx != c->curr_width || cy != c->curr_height) {
		...
		c->sws = sws_getContext(c->v_width, c->v_height, AV_PIX_FMT_YUV420P,
